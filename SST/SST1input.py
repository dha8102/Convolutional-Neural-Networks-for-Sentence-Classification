...
def make_embeddings():
    ...
    for i in reverse_voca.keys():
        if reverse_voca[i] in word2vec.keys():
            ...
            for j in range(300):
                embeddings[i][j] = np.random.uniform(-0.2, 0.2)
    print("precnt:", precnt)
    return embeddings
...
def inputdata():
    file = ["./dataset/stsa.fine.phrases.train",
            "./dataset/stsa.fine.test",
            "./dataset/stsa.fine.dev"]
    # voca 생성
    max_sentence_len = 0
    voca = {}
    vocacnt = 0
    ...
    print("Total number of sentences : ", sentencecnt)
    ...
    y_test = []
    x_cnt = 0
    filename = "./dataset/stsa.fine.test"
    with open(filename, encoding='latin-1') as f:
        while True:
            line = f.readline()
            if not line:
    ...
    y_dev = []
    x_cnt = 0
    filename = "./dataset/stsa.fine.dev"
    with open(filename, encoding='latin-1') as f:
        while True:
            line = f.readline()
            if not line:
...
if __name__ == "__main__":
    ...
    f = open("./SST1_original_data.bin", "wb+")
    pickle.dump(total_data, f)
    f.close()
    print("SST1_original_data produced.")
